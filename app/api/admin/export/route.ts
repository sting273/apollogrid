...
export async function GET() {
  ...
  const comparisonHeaders = ["id","created_at","address","postcode","proposal_url","our_usage_kwh","pylon_annual_usage_kwh","our_panel_count","pylon_panel_count","our_generation_kwh","pylon_generation_kwh","generation_conservatism_pct","our_bill_before","pylon_bill_before","our_bill_after","pylon_bill_after","our_benefit","pylon_benefit","benefit_conservatism_pct","input_hit_rate","generation_hit_rate","benefit_hit_rate","overall_hit_rate"];
  const apiHeaders = ["id","created_at","provider","endpoint","status_code","success"];
  const numeric = new Set(["annual_usage_kwh","panel_count","panel_watts","system_kwp","annual_generation_kwh","annual_bill_before","annual_bill_solar_only","annual_bill_solar_battery","annual_benefit","consent","our_usage_kwh","pylon_annual_usage_kwh","our_panel_count","pylon_panel_count","our_generation_kwh","pylon_generation_kwh","generation_conservatism_pct","our_bill_before","pylon_bill_before","our_bill_after","pylon_bill_after","our_benefit","pylon_benefit","benefit_conservatism_pct","input_hit_rate","generation_hit_rate","benefit_hit_rate","overall_hit_rate","status_code","success"]);
  const comparisonRows=comparisons.results.map(record=>{const pylonGeneration=Number(record.pylon_generation_kwh||0),pylonBenefit=Number(record.pylon_benefit||0);return {...record,generation_conservatism_pct:pylonGeneration?(pylonGeneration-Number(record.our_generation_kwh||0))/Math.abs(pylonGeneration)*100:0,benefit_conservatism_pct:pylonBenefit?(pylonBenefit-Number(record.our_benefit||0))/Math.abs(pylonBenefit)*100:0};});
  const workbook = `<?xml version="1.0"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Styles><Style ss:ID="header"><Font ss:Bold="1" ss:Color="#FFFFFF"/><Interior ss:Color="#F15B32" ss:Pattern="Solid"/></Style></Styles>${sheet("Assessments",assessmentHeaders,toRows(assessments.results,assessmentHeaders,numeric))}${sheet("Leads",leadHeaders,toRows(leads.results,leadHeaders,numeric))}${sheet("Pylon Comparison",comparisonHeaders,toRows(comparisonRows,comparisonHeaders,numeric))}${sheet("API Usage",apiHeaders,toRows(apiUsage.results,apiHeaders,numeric))}</Workbook>`;
