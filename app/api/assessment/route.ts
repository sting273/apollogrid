...
async function lookupAddresses(postcode: string, fallbackLatitude: number, fallbackLongitude: number) {
  ...
  const response = await fetch(`https://api.ideal-postcodes.co.uk/v1/postcodes/${encodeURIComponent(postcode)}?api_key=${encodeURIComponent(apiKey)}`, { signal: AbortSignal.timeout(15000) });
  await recordApiUsage("Ideal Postcodes", "Address lookup", response.status, response.ok);
  const payload = await response.json() as { code?: number; result?: Array<{ line_1?: string; line_2?: string; line_3?: string; post_town?: string; postcode?: string; latitude?: number; longitude?: number }> };
  if (!response.ok || (payload.code !== undefined && payload.code !== 2000)) {
    const notFound = payload.code === 4040;
    return { configured: true, status: notFound ? "no_results" : "provider_error", providerCode: payload.code, error: notFound ? null : "Automatic address lookup is temporarily unavailable. Enter your address manually or try again later.", addresses: [] as AddressResult[] };
  }
  const addresses = (payload.result ?? []).map((item) => {
    ...
    return {
      ...
      latitude: item.latitude != null && Number.isFinite(latitude) ? latitude : fallbackLatitude,
...
export async function GET(request: Request) {
  ...
  const [electricity, addressLookup] = await Promise.all([
    ...
    lookupAddresses(locationPayload.result.postcode, locationPayload.result.latitude, locationPayload.result.longitude).catch(() => ({ configured: Boolean(providerSecret("IDEAL_POSTCODES_API_KEY", process.env.IDEAL_POSTCODES_API_KEY)), status: "provider_error", error: "Automatic address lookup is temporarily unavailable. Enter your address manually or try again later.", addresses: [] as AddressResult[] })),
  ]);
