...
type Lookup = {
  ...
  addressLookup: { configured: boolean; status?: string; error?: string | null; addresses: AddressOption[] };
};
...
export default function Home() {
  ...
  const persistAssessment = async (id: string) => {
    const payload = await saveAssessment({
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { persistAssessment(assessmentId).catch(() => setSaveError("Your estimate has not been saved yet. Please retry.")); }, 350);
  ...
  const calculateSavings = async () => {
    ...
    } catch { setSaveError("Your estimate has not been saved yet. Please retry."); }
  };
  ...
  if (stage === "result") return <main className="app-shell result-bg">
    ...
    <section className="result-step">
      {saveError && <div role="alert" className="lookup-error">{saveError} <button onClick={calculateSavings}>Retry saving</button></div>}
